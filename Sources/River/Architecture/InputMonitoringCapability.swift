...
final class InputMonitoringCapability: Capability {
    ...
    let setupInstructions: String? =
        "Open System Settings → Privacy & Security → Input Monitoring, click +, add /Applications/River.app, and toggle it on. Then click Refresh permission status below — if it still shows Not granted, quit and relaunch River."
    ...
    /// Starts the system-wide CGEventTap on a dedicated `com.river.eventtap`
    ...
    func startTap() async {
        ...
        let started: Bool = await withCheckedContinuation { (cont: CheckedContinuation<Bool, Never>) in
            ...
            thread.name = "com.river.eventtap"
        ...
        tapLogger.info("CGEventTap started on com.river.eventtap")
    }
