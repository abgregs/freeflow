...
/// Pure mapping from `RiverError` kind → toast presentation, exactly like
/// `MenuBarPresentation.visual`. Structures the *presentation*, not the error: the
/// typed `RiverError` taxonomy is unchanged (planning 0018). Every case maps, so
/// a new error stage is a compile error here, not a silently missing toast.
enum ErrorToastPresentation {
    static func toast(for error: RiverError) -> ErrorToast {
        switch error {
        case .audioCapture:
            return ErrorToast(
        ...
        case .textInsertion:
            return ErrorToast(
                ...
                hint: "Click into a text field and try again — River needs Accessibility access."
            )
        }
    }
}
