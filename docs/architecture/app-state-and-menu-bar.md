...
final class AppState {
    ...
    func bind(to session: RiverSession)   // sinks session.state + session.errors
}
```
...
`RiverApp`'s `MenuBarExtra` label and content both render from `MenuBarPresentation.visual(state:hasError:)`, with the error message shown as a second menu line when present.
