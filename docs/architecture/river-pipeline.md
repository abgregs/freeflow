...
If `AccessibilityCapability.postKeyEvent` throws (status not `.granted`, or the silent-no-op detector fires), `TextInsertionManager` propagates the error to `RiverSession`. The session logs at `.error`, updates `appState.errorMessage`, and returns to `.idle`. The user sees the error reflected in the UI; nothing fails silently.
