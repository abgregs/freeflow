...
If `AccessibilityCapability.postKeyEvent` throws (status not `.granted`, or the silent-no-op detector fires), `TextInsertionManager` restores the original pasteboard immediately, rethrows, and `RiverSession.handleDeactivate` catches the error, logs at `.error` (path-redacted per [ADR 0002](../decisions/0002-log-redaction-over-debug-flag.md)), and returns to `.idle`. There are now **two** user-visible surfaces:
