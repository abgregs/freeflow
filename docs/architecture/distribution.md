...
- **Recovery protocol for a dev rebuild that loses Accessibility:** quit the app → `tccutil reset Accessibility com.river.app` (removes the stale row) → System Settings → Accessibility → **+** → add `/Applications/River.app` fresh → relaunch. Toggling the existing stale row is a no-op; it must be removed/re-added.
