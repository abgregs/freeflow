...
- **Pane-added Accessibility rows survive same-identity rebuilds too — but the app *reads* denied at launch.** Repeated rebuilds (2026-08-19) showed the lighter recovery works every time: Grant (Settings shows the row already on) → back → Refresh → granted. No reset, no re-add. So the row persists; the launch-time "not granted" is `AXIsProcessTrusted()` returning false for the rebuilt binary. **What clears it is not yet pinned down**: one observation needed the Accessibility pane opened, another cleared with a single Refresh after a delay, so elapsed time is the likelier factor (2026-09-14; see 0012 section 5, which has the test that settles it). It is not a probe race: 0012's probe retry-with-settle (PR #33) produced no change. Until that lands: after each `make install`, press Refresh in onboarding; if it still reads denied, press Grant (opens the pane), come back, and Refresh again.
- **Genuinely stale rows are the heavy case.** A row created against a *different* identity (the Homebrew-era build, or any identity churn) shows enabled in Settings while tccd refuses the running binary, and Refresh never helps. Recovery: quit → `tccutil reset Accessibility com.freeflow.app` → re-add fresh via **+** → relaunch. Toggling the stale row is a no-op.
