...
4. **Text appears at the cursor.** Synthesized ⌘V pastes the transcription into whatever text field currently has focus. The user's original clipboard is restored after the paste lands. When the focused element is clearly non-editable (a focused button, a selected list row or DOM node), the paste is skipped *before* the clipboard is touched and the menu bar shows "No text field focused"; an ambiguous target attempts the paste (fail-open). See the paste guard in [../architecture/free-flow-pipeline.md](../architecture/free-flow-pipeline.md).
