...
cd ~/Developer/free-flow-stack/<worktree>
...
`make install` does `rm -rf` on `/Applications/FreeFlow.app`. Doing that to a running app deletes the bundle out from under a live process, and since the silent-no-op detector keys off bundle identity, that is a good way to manufacture a confusing false negative that looks like a permissions regression.

### Clean slate before install

If the Homebrew cask is installed, `/Applications/FreeFlow.app` belongs to it. Removing it first eliminates any ambiguity about which binary is running and which bundle holds the TCC grants. Cheap step; do it first rather than debugging a shadowed build later.

### Expect one permission round-trip per rebuild

Each `make install` re-signs with the self-signed "Free Flow Dev" identity, so macOS treats the bundle as a changed binary. Accessibility typically needs one Grant → Refresh round-trip before the app's own launch-time read reports `.granted`. The TCC row survives; the app-side read is the false negative. This is what planning 0012 (PR #33) addresses — until it lands, treat one round-trip as expected, not as a regression.
...
It **never touches** `~/Library/Application Support/FreeFlow`. Downloaded models always persist across installs — the sandbox is intentionally off, so the cache lives outside the app container (see [../architecture/distribution.md](../architecture/distribution.md)).
...
find ~/Library/Application\ Support/FreeFlow -name "*.incomplete" -o -name "*.part"

# Has anything been written recently? (no output = nothing is downloading)
find ~/Library/Application\ Support/FreeFlow -type f -newermt '-1 hour'

# What is the process actually doing?
sample $(pgrep -f "FreeFlow.app/Contents/MacOS/FreeFlow") 3
```
...
```bash
defaults write com.freeflow.app selectedModel openai_whisper-base.en
```
