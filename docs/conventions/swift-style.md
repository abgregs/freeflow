...
- **Types**: `UpperCamelCase`. The cycle's state-owning collaborators under `Managers/` use the `Manager` suffix uniformly — `HotkeyManager`, `AudioCaptureManager`, `TextInsertionManager`, and `TranscriptionManager`. Pure value types and stateless helpers use behavioral names without a suffix (e.g., `TapEvent`, `OnboardingGate`, `SystemSettingsPane`). **Why:** consistency across the `Managers/` folder beats per-name micro-optimization; the suffix signals "owns significant state and an OS/network surface" at a glance.
