...
These docs are the source of truth for how River is built and maintained. Before a non-trivial change, surface the applicable conventions from these docs; afterwards, check them for staleness. The repo's `brief` and `debrief` skills (`.claude/skills/`) automate both for Claude Code users.
