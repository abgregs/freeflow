...
These docs are the source of truth for how River is built and maintained. The working convention: before a non-trivial change, surface the applicable conventions from these docs. Contributing requires no particular agent, harness, or tooling. Doc updates alongside a change are welcome, not a requirement — doc/code drift is caught by the maintainer's automated daily doc sync ([conventions/doc-maintenance.md](conventions/doc-maintenance.md)).
