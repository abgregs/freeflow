...
These docs are the source of truth for how River is built and maintained. The working convention: before a non-trivial change, surface the applicable conventions from these docs; afterwards, check them for staleness. The repo's `brief` and `debrief` skills (`.claude/skills/`) automate both for Claude Code users, but they are optional tooling — contributing requires no particular agent, harness, or skill. Doc updates alongside a change are welcome, not a requirement; the maintainer audits doc health and syncs as needed.
