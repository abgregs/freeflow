...
This does **not** violate "No AX-API path" ([river-pipeline.md](../architecture/river-pipeline.md)): that decision rejected AX *writes* (`AXUIElementSetAttributeValue`); this is input-event synthesis (`CGEvent`), the same family as the ⌘V it replaces.
...
- [../architecture/river-pipeline.md](../architecture/river-pipeline.md) — the insertion step and the "No AX-API path" decision
