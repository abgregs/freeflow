...
WhisperKit's default download location is `~/Documents/huggingface`. Because the app is unsandboxed, the first time it writes there macOS shows a **"River wants to access files in your Documents folder"** TCC prompt — confusing for a dictation app — and it leaves a `huggingface` folder cluttering the user's Documents. Surfaced during the `v0.0.1-rc3` on-device test.

## Fix

Pass a `downloadBase` to `WhisperKit(model:downloadBase:load:)` pointing at **`~/Library/Application Support/River`**. Application Support is **not** TCC-protected, so:
...
1. On a machine with no prior cache, first launch downloads the model into `~/Library/Application Support/River/…` with **no Documents-folder prompt**, and dictation works.
...
- [../architecture/river-pipeline.md](../architecture/river-pipeline.md) — where the transcription/model-load step sits
