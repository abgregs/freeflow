...
[M6: Transcription](milestones.md#m6-transcription). Integrate WhisperKit. `TranscriptionService.transcribe(audioSamples:)` returns text or a typed error; default model loads on its own schedule (not blocking the session). Custom dictionary integration with the special-token filter (`tokenizer.specialTokens.specialTokenBegin`) in place. `RiverSession` calls the service during `.processing` (replacing M5's sample-count log) and continues to return to `.idle`. Run `/brief` before starting.
