...
Ordered. Each milestone is a state the project can pause at without being broken. Milestones are *outcomes*, not task lists — whoever picks one up decomposes it into tasks after surfacing the applicable conventions from `docs/`.
...
**Status (as of 2026-06-22):** **M1–M11 are complete — River shipped `v0.1.0`.** The release pipeline ran end-to-end (Developer-ID-signed, notarized, stapled DMG + SHA-256 published to the GitHub Release) and the Homebrew tap (`abgregs/homebrew-river`) is live and verified. The V1 sprint is done; further work is the post-V1 backlog in [_index.md](_index.md).

## M1: Walking skeleton

A signed, installable menu bar app that does nothing useful. Proves the build, sign, and install pipeline before any feature code is written. Also establishes the architectural skeleton: empty `Capability` protocol declarations, an empty `SettingsStore`, a stub `RiverSession` that does nothing. See [walking-skeleton.md](walking-skeleton.md) for the detailed acceptance criteria.
...
## M3: RiverSession skeleton

[`RiverSession`](../architecture/river-session.md) exists with the public interface (`start()` / `stop()` / `state` publisher). It owns the `RiverState` machine, holds references to (empty stub) managers, and subscribes to [`SettingsStore`](../architecture/settings-store.md) publishers — even though the store has no keys yet beyond placeholders. `AppDelegate` shrinks to construct-and-start.

Exit criteria: launching the app yields a menu bar with state visible via `RiverSession.state.idle`. No actual dictation happens; the wiring is the test surface. A unit test drives `session.start()` and asserts the initial state.
...
`MicrophoneCapability` owns audio-engine start/stop. `AudioCaptureManager` records hardware-format audio while held; converts to 16 kHz mono Float32 on stop. Handles the engine-warmup race (wait for first buffer before teardown). `RiverSession` calls into the manager during `.recording`.
...
WhisperKit integrated. Default model loads on launch (on its own schedule, not blocking the session). `TranscriptionService.transcribe` returns text or a typed error. Custom dictionary integration with the special-token filter in place. `RiverSession` calls into the service during `.processing`.
