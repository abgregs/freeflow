...
Exit criteria: `make install` produces a working `/Applications/River.app` that survives across rebuilds without re-granting permissions. The bundle-misidentification failure mode is detected by `AccessibilityCapability` at runtime if it ever occurs.

## M11: Public release pipeline

GitHub Action: on tag, build with Developer ID signing, notarize via `notarytool`, staple, package as `.dmg`, attach to release. Homebrew cask in a separate `homebrew-river` tap repo pointing at the release artifact.
