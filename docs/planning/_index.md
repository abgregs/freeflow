...
- [0006_runtime-security-hardening.md](0006_runtime-security-hardening.md) — runtime findings from the same review: the event tap `.listenOnly` hardening (least privilege) **landed 2026-06-17** (on-device verify rides the pre-v0.1.0 rc), plus the model-cache / no-sandbox trade-offs on record (the pasteboard-transit trade-off was eliminated by 0011)
