...
- [0006_runtime-security-hardening.md](0006_runtime-security-hardening.md) — runtime findings from the same review: the event tap `.listenOnly` hardening (least privilege) **landed 2026-06-17** (on-device verify rides the pre-v0.1.0 rc), plus the accepted pasteboard / model-cache / no-sandbox trade-offs on record
...
- [0010_relocate-model-cache.md](0010_relocate-model-cache.md) — **landing before v0.1.0**: download the WhisperKit model under `~/Library/Application Support/River` instead of WhisperKit's `~/Documents` default, so first launch doesn't trigger a Documents-folder TCC prompt or clutter the user's Documents.
