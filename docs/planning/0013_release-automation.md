...
2. **The Homebrew cask** is bumped by hand: edit `version` + `sha256` in [`packaging/homebrew/river.rb`](../../packaging/homebrew/river.rb), copy it into the separate tap repo (`abgregs/homebrew-river`), and push.
...
The recurring question is whether the cask can live in the main `river` repo instead of `homebrew-river`. It can't, and shouldn't:

- **The `homebrew-` prefix is mandatory.** `brew install --cask abgregs/river/river` resolves the tap `abgregs/river` to the repo `abgregs/homebrew-river` — Homebrew hardcodes that prefix. A tap cannot be served from the `river` app repo. (The prefix is also what keeps the app repo and the tap from colliding even though both end in `river`.)
- **`brew tap` clones the entire tap repo to every user's machine.** A tiny cask-only repo is the correct design; folding it into `river` would clone the whole app history onto each installer.
...
- **Source-of-truth split:** `packaging/homebrew/river.rb` stays the *reviewed template* — it owns the cask's **shape** (`name`/`desc`/`depends_on`/`caveats`/`zap`/`url` pattern). The workflow substitutes the only two generated fields — `version` (from the tag) and `sha256` (read from the published `.dmg.sha256`, not recomputed) — and writes the result to `Casks/river.rb` in the tap. **Why:** the cask's structure is what deserves PR review in the main repo; `version`/`sha256` are mechanical. This removes the "copy across" step while keeping the template meaningful.
...
4. The cross-repo token is fine-grained, scoped to `homebrew-river` `contents:write` only.
5. `packaging/homebrew/river.rb` remains the reviewed source of the cask's shape; only `version`/`sha256` are generated.
