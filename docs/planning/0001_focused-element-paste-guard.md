...
When the guard skips, it shows "No text field focused" via the menu-bar label / session-level error publisher rather than doing nothing visibly. That surface was built by the **Menu-bar visual-state** milestone (the `RiverSession.errors` publisher + the `AppState` bridge, now landed — see [../architecture/app-state-and-menu-bar.md](../architecture/app-state-and-menu-bar.md)). Reusing it, instead of inventing a one-off signal here, is the reason this milestone is sequenced after it.
...
- [../architecture/river-pipeline.md](../architecture/river-pipeline.md) — "No AX-API path", the decision this read-only check refines
