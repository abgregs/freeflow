...
2. **Bundle integrity** — `Info.plist` and `--entitlements` are mandatory at sign time. `make verify` (run by `make install` and the release workflow) asserts the signed bundle identifier and fails loudly. There is no runtime detector for a malformed bundle, so it must never get past the build. See [docs/conventions/anti-patterns.md](docs/conventions/anti-patterns.md) item #3.
