...
struct RiverSessionTests {
    ...
    func tapModeKeyChangeAppliesLive() async throws {
        ...
        // notice tells the user the new key now stops it. See river-session.md.
        let env = makeSession()
        try await env.session.start()
