...
/// only when `RIVER_EVAL_CORPUS` (a path to a local corpus directory) is set,
...
/// those; e.g. `~/river-eval-corpus`):
///
///   RIVER_EVAL_CORPUS="$HOME/river-eval-corpus" \
///     swift test --filter TranscriptionEval
///   open -e eval-openai_whisper-small.en.txt
///
///   Optional: RIVER_EVAL_MODEL=openai_whisper-large-v3-turbo  (default: small.en)
///             RIVER_EVAL_OUT=/tmp/scorecard.txt               (default: ./eval-<model>.txt)
@Suite("TranscriptionEval")
struct TranscriptionEvalTests {
    @MainActor
    @Test(
        "corpus WER + latency scorecard for one model",
        .enabled(if: ProcessInfo.processInfo.environment["RIVER_EVAL_CORPUS"] != nil)
    )
    func evalCorpusScorecard() async throws {
        let env = ProcessInfo.processInfo.environment
        let corpusPath = try #require(env["RIVER_EVAL_CORPUS"], "set RIVER_EVAL_CORPUS to a corpus directory")
        let model = env["RIVER_EVAL_MODEL"] ?? Constants.defaultModel
        ...
        let outPath = env["RIVER_EVAL_OUT"] ?? "eval-\(model).txt"
...
private enum EvalCorpusError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingDirectory(let path):
            return "RIVER_EVAL_CORPUS directory does not exist: \(path)"
...
private enum Scorecard {
    static func render(
        ...
        var lines: [String] = []
        lines.append("River transcription eval — \(model)")
