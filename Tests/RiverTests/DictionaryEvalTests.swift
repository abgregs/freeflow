...
/// only when `RIVER_AB_WAV` (a path to a recorded clip) is set, so the normal
...
///
///   RIVER_AB_WAV="$PWD/clip.m4a" RIVER_AB_TERM="Vite" \
///     swift test --filter DictionaryEval
///   open -e ab-result.txt          # or just open it in your editor
///
/// Optional: RIVER_AB_MODEL=openai_whisper-base.en  (default: small.en)
@Suite("DictionaryEval")
struct DictionaryEvalTests {
    @MainActor
    @Test(
        "A/B: same clip, dictionary on vs off",
        .enabled(if: ProcessInfo.processInfo.environment["RIVER_AB_WAV"] != nil)
    )
    func dictionaryABOnFixedClip() async throws {
        let env = ProcessInfo.processInfo.environment
        let wavPath = try #require(env["RIVER_AB_WAV"], "set RIVER_AB_WAV to a recorded clip path")
        let term = env["RIVER_AB_TERM"] ?? "Vite"
        let model = env["RIVER_AB_MODEL"] ?? Constants.defaultModel
        ...
        let outPath = env["RIVER_AB_OUT"] ?? "ab-result.txt"
