...
struct ErrorToastPresentationTests {
    // The error-kind → toast mapping is a pure function covering every RiverError
    ...
    @Test("every RiverError kind maps to a non-empty headline and recovery hint")
    func everyCaseHasHeadlineAndHint() {
        let dummy = NSError(domain: "x", code: 1)
        let errors: [RiverError] = [
