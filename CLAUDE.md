...
Authoritative project context lives in [`docs/`](docs/_index.md). Read `docs/_index.md` first to get the map.

**Before any non-trivial code change:** invoke the `/brief` skill. It will surface applicable conventions, architectural constraints, and active requirements from `docs/`.

**After any non-trivial code change:** invoke the `/debrief` skill. It checks docs for staleness and proposes updates when code introduces a new pattern or replaces an existing convention.

If you're starting fresh and the codebase is empty, the first task is [M1: Walking Skeleton](docs/planning/walking-skeleton.md). The agent picking up this work should read `docs/planning/current-focus.md` first.
...
The `make install` step requires a one-time self-signed code-signing certificate named "River Dev" in the login keychain. See [docs/architecture/distribution.md](docs/architecture/distribution.md).
